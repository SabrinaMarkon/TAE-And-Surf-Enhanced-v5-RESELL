...
if( session_is_registered("alogin") ) {
        ...
        echo "<center><H2>All Active Traffic Links</H2></center>";
      $query = "SELECT COUNT(*) as num FROM tlinks where approved = 1";
                  $total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];

        $query = "select * from tlinks where approved = 1";
		$result = mysql_query ($query)
	     	or die ("Query failed");
           echo "<font size=2 face='$fonttype' color='$fontcolour'><p><center>";
    echo "<center><p><b>";
   echo "$total_pages Active Traffic Links Found";
    echo "</center></p></b>";
mysql_close($dblink);
    ?>
        <center>
						<form method="POST" action="deletetlinks.php">
